...
class Employee_model extends CI_Model
{
    ...
    public function update_employee($id, $name, $phone, $phonealt, $address, $city, $region, $country, $postbox, $location, $salary = 0, $department = -1,$commission=0,$roleid=false,$service,$daysTime)
    {
        ...
        $data = array(
            ...
            'sun_to'=>$daysTime['sun_to'],
            'mon_from'=>$daysTime['mon_from'],
            'mon_to'=>$daysTime['mon_to'],
            'tue_from'=>$daysTime['tue_from'],
            'tue_to'=>$daysTime['tue_to'],
            'wen_from'=>$daysTime['wen_from'],
            'wen_to'=>$daysTime['wen_to'],
            'the_from'=>$daysTime['the_from'],
            'the_to'=>$daysTime['the_to'],
            'fri_from'=>$daysTime['fri_from'],
            'fri_to'=>$daysTime['fri_to']
        );
        if ($department > -1) {
            $data = array(
                'name' => $name,
    ...
    public function addnewRole($name)
    {
        ...
        if ($this->db->insert($this->DBname_roles, $data)) {
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('ADDED'). "  <a href='".base_url('employee/roles')."' class='btn btn-blue btn-lg'><span class='fa fa-list-alt' aria-hidden='true'></span>  </a> <a href='add' class='btn btn-info btn-lg'><span class='fa fa-plus-circle' aria-hidden='true'></span>  </a>"));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
                $this->lang->line('ERROR')));
        }
    ...
    public function editRole($id, $name)
    {
        ...
        if ($this->db->update($this->DBname_roles)) {
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('UPDATED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
    ...
    {
            if ($this->db->delete($this->DBname_roles, array('id' => $id))) {
                return true;
            } else {
                return false;
            }
