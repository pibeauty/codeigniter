...
{
    var $table = 'geopos_purchase';var $tablePayment = 'payment_tb';var $tableCreditDebit = 'create_debit_note';
    ...
    public function get_purchases($user_id){
        $this->db->select('geopos_purchase.id,geopos_purchase.user_id,geopos_purchase.supuser_id,geopos_purchase.tid,geopos_purchase.invoicedate,geopos_purchase.invoiceduedate,geopos_purchase.total,geopos_purchase.status,geopos_supplier.name');
        $this->db->from($this->table);
        $this->db->where('geopos_purchase.user_id', $user_id);
        $this->db->join('geopos_supplier', 'geopos_purchase.csd=geopos_supplier.id', 'left');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function check_orders_by_date($user, $fromDate, $toDate,$purch_sale){
        $this->db->select('*');
        $this->db->from('geopos_purchase');
        $this->db->where('user_id', $user);
        $this->db->where('purch_sale', $purch_sale);
        $this->db->where('invoicedate >=', $fromDate);
        $this->db->where('invoicedate <=', $toDate);
        $this->db->where('status <>', 'D,O');
        $query = $this->db->get();
        $result = $query->result();
        return $result;
    }
    public function check_orders_by_DeliveryOrder($user, $fromDate, $toDate,$purch_sale){
        $this->db->select('*');
        $this->db->from('geopos_purchase');
        $this->db->where('user_id', $user);
        $this->db->where('purch_sale', $purch_sale);
    ...
    public function check_orders_by_user($user){
        $this->db->select('*');
        $this->db->from('geopos_purchase');
        $this->db->where('user_id', $user);
        $query = $this->db->get();
...
public function savePayment($data){
            ...
            $this->db->insert($this->tablePayment, $data);

            return "ok";
}
    public function saveCreditDebit($data){

        $this->db->insert($this->tableCreditDebit, $data);
    ...
    public function ConvertToDO($id){
        $this->db->set(['status' =>'D,O']);
