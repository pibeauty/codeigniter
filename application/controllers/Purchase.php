...
class Purchase extends CI_Controller
{
    ...
    {
        if ($this->input->get('idsupplier')) {
            $idSupUser = $this->input->get('idsupplier');
            $query_stock = $this->db->select('id,product,quantity,user_id')->from('application_user_stocks')->where('user_id', $idSupUser)->get();
            $results_stock = $query_stock -> result_array();
            $results_stock_product=array();
            $results_product_items=array();
            $results_items_details=array();
            foreach($results_stock as $result){
                $query_product = $this->db->select('pid,pcat,product_name,product_price,product_des,image')->from('geopos_products')->where('pid', $result['product'])->get();
                ...
                $result["product-details"]=$results_product_items;
                array_push($results_stock_product,$result);
            }
            // echo json_encode(array('List Stocks & Products & ProductItems' => $results_stock_product));
            echo "<h2>List Stocks & Products & ProductItems</h2>"."<br>";
            echo "<pre>"; print_r($results_stock_product);echo "</pre>";
        } else{
            echo json_encode(array('status' => 'is not id supuser'));
        }
    }
