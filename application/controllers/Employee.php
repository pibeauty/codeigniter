...
class Employee extends CI_Controller
{
    ...
    public function submit_user()
    {
       ...
       // log_message('error',"------------------------------------ghjghacascsascj:". implode(",",$this->input->post('service')));
        ...
        $daysTime=array(
            ...
            'sun_from'=>$this->input->post('sun_from', true),
            'sun_to'=>$this->input->post('sun_to', true),
            'mon_from'=>$this->input->post('mon_from', true),
            'mon_to'=>$this->input->post('mon_to', true),
            'tue_from'=>$this->input->post('tue_from', true),
            'tue_to'=>$this->input->post('tue_to', true),
            'wen_from'=>$this->input->post('wen_from', true),
            'wen_to'=>$this->input->post('wen_to', true),
            'the_from'=>$this->input->post('the_from', true),
            'the_to'=>$this->input->post('the_to', true),
            'fri_from'=>$this->input->post('fri_from', true),
            'fri_to'=>$this->input->post('fri_to', true),
        );
    ...
    public function invoices_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            $row[] = amountExchange($invoices->employeesshare, 0, $this->aauth->get_user()->loc);
    ...
    public function roles(){
        ...
        $data['roles'] = $this->employee->list_roles();
        $this->load->view('fixed/header', $head);
        $this->load->view('employee/index_roles', $data);
        $this->load->view('fixed/footer');
    }
    public function addRole(){
        $head['title'] = "Add Role";
        $this->load->view('fixed/header', $head);
        $this->load->view('employee/role-add');
        $this->load->view('fixed/footer');
    }
    public function add_new(){
        $name = $this->input->post('name');
        echo   $this->employee->addnewRole($name);
    }

    public function editRole(){
        $id = $this->input->get('id');
        $data['role'] = $this->employee->detailsRole($id);
        $head['title'] = "Edit Role";
        $this->load->view('fixed/header', $head);
        $this->load->view('employee/role-edit',$data);
        $this->load->view('fixed/footer');
    }
    public function updateRole(){
        $name = $this->input->post('name');
        $id = $this->input->post('id');
        echo   $this->employee->editRole($id,$name);
    }
    public function deleteRole(){
        $id = $this->input->get('id');
        $this->employee->deleteRole($id);
