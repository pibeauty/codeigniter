...
class Appusers extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $head['title'] = 'Application Users';
        $data['employee'] = $this->employee->list_employee();
        $this->load->view('fixed/header', $head);
        $this->load->view('application/users/list');
...
//        $data['dept'] = $this->employee->department_list(0);
        ...
        $head['title'] = 'Create Application User';
        $this->load->view('fixed/header', $head);
        $this->load->view('application/users/create', $data);
        $this->load->view('fixed/footer');
    }
    ...
    public function submit_user()
    {
        ...
        $data['postbox'] = $this->input->post('postbox', true);
        $data['phone'] = $this->input->post('phone', true);
        $data['salary'] = numberClean($this->input->post('salary', true));
        $data['commission'] = $this->input->post('commission', true);
        ...
        try {
            ...
            echo json_encode(array('status' => 'Error', 'message' =>
                'There has been an error, please try again.'.$e->getMessage()));
        }
    }

    public function checkAvailability(){
        $username = $this->input->post('username');
        $result = $this->users->username_exist_in_database($username);
