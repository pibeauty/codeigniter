...
class Salesorder Extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('products_model','prodmod');
        $this->li_a = 'stock';
    }

    public function index()
    {
        $data['sales'] = $this->salesorder->sales_orders();
        $head['title'] = "Sales Order";
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('salesorder/management', $data);
...
//        if ($this->aauth->premission(11)) {
            ...
            if ($id) {
                $this->db->delete('sales_order', array('id' => $id));
                echo json_encode(array('status' => 'Success', 'message' => 'Order Deleted Successfully!'));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => 'Sorry No Order Found!'));
            }
//        } else {
//            echo json_encode(array('status' => 'Error', 'message' =>
//                $this->lang->line('ERROR')));
