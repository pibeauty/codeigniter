...
$('#addproduct').on('click', function () {
    ...
    $.ajax({
        url: baseurl+'employee/createEmployeeSelectOption',
        type: 'GET',
        async: false,
        fail: function(){
            alert('fail');
    ...
    $.ajax({
        url: baseurl+'services/createServicesSelectOption',
        type: 'GET',
        async: false,
        fail: function(){
            alert('fail');
...
$(document).on('click', ".v2_select_pos_item", function (e) {
    ...
    var flag = true;
    $('#v2_search_bar').val('');
    console.log("oldis_qrcode:"+is_qrcode+"///////pcatInlen:"+$('.pcatIn').length);
    if(pcat==2){
        if(is_qrcode==0) {//pre product was not qr mode
            if($('.pcatIn').length==0){//the first item
                is_qrcode=1;
            }
            else {
                flag = false;
        ...
        if(is_qrcode==1){//pre product was qr mode
            flag = false;
            alert("no");
        }
        else if(is_qrcode==0){//pre product was not qr mode
            is_qrcode=0;
        }

    }
   // console.log("length:"+$('.pcatIn').length+"/////lengthpid:"+pid);

    $('.pcatIn').each(function () {
       // console.log($(this).val()+"/////pcat:"+pcat);
        if(!is_qrcode){
            if ($(this).val() !=2) {
             //   flag = false;
            }
